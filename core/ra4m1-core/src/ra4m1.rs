...
mod tests {
    ...
    fn ra4m1_map_dtc_repeat() {
        ...
        // transfer_info_t: num_blocks u16 @+12, length u16 @+14.
        // The high half of the word at +12 IS length @+14 (R_DTC
        // doubles it into CRAL/CRAH on the wire, so live reads like
        // 0x1818 mask to CRAL for the count).
        mem.write32(0x2000_110C, 2 << 16); // length = 2 (high half)
    ...
    fn ra4m1_pwm_ok() {
        ...
        // (The real analogWrite() path is proven separately in
        // analogwrite_ok - this stays bare-metal for the waveform.)
        const APP_BASE: u32 = 0x4000;
        let _g = RA_BOOT_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    ...
    // CPU: predicated 16-bit flag-setting preserves flags (printNumber's
    // `ite le; addle r3,#48; addgt r3,#55` must not execute both arms:
    // addle must not kill N before addgt's test). ARM ARM: 16-bit insns
    // in an IT block, other than CMP/CMN/TST, do NOT set flags. GAS
    // vectors in core/docs/it12.s (assemble with the F4 toolchain flags
    // in core/docs/README.md): A ite-le imm3 (writeback happens, flags
    // untouched), B itt-mi skipped (regs + flags unchanged), C cmpeq
    // SETS flags / addne preserves, D shifts preserve, E AND/ORR
    // preserve, F tsteq SETS flags. Mirrors the existing
    // MOVS/ADD-reg/SUB-reg it_pred rule in the decoder.
    #[test]
