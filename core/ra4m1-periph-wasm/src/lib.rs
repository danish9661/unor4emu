...
/// reports it once. Same name as the legacy STM32-path helper below so
/// runners use one spelling; kept under both names for compat.
#[wasm_bindgen]
pub fn dma_component_completed(stream_idx: u32, success: bool) {
    sys().mark_dma_completed(stream_idx as usize, success)
}
