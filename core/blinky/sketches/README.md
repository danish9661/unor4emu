...
- **Exact** (recompile to a working equivalent): r4aw, r4aws, r4tone, r4sser,
  ...
  r4canfifo, r4canerr, r4analog, r4matrix. Verified: `arduino-cli compile --fqbn
  arduino:renesas_uno:minima` passes for each (36/36 dirs; r4mtx needs
  `--fqbn arduino:renesas_uno:unor4wifi` instead — matrix pins D28–D38
  exist only in the WiFi variant's `g_pin_cfg`, same RA4M1 silicon).
- **Shape** (shows the Arduino-level flow; vendored bin stays the proof
  ...
  r4matrix (bare-metal register flows per the test comments; rebuilt once
  against the settled PCNTR1 layout PDR-low/PODR-high with
  level-before-direction ordering so no phantom LED conducts),
...
done` — 36 pass (incl. r4aws, r4rtcalm, r4canbo, r4opamp, r4analog,
r4matrix; r4mtx passes with the unor4wifi fqbn), r4ctsu (no FspTouch.h
